...
from odoo import api, fields, models, _ , SUPERUSER_ID, exceptions
...
class SprogroupPurchaseRequest(models.Model):
    ...
    line_ids = fields.One2many('sprogroup.purchase.request.line', 'request_id','Products to Purchase', readonly=False,copy=True, track_visibility='onchange')          
    ...
    owner = fields.Many2one('hr.employee', string='Requested by', readonly=True)
    priority = fields.Selection([('0', 'Very Low'), ('1', 'Low'), ('2', 'Normal'), ('3', 'High')], string='Priority')
    equipment_id = fields.Many2one('maintenance.equipment', string='Equipment', ondelete='restrict', index=True)
    department_id = fields.Many2one('hr.department', string='Department',  index=True, ondelete='restrict')
    request_site = fields.Many2one('hr.department', string='Site',  index=True, ondelete='restrict')
    ...
    def create(self, vals):
        ...
        if request.assigned_to.has_group('sprogroup_purchase_request.group_sprogroup_purchase_request_approver') == False :
            raise exceptions.ValidationError('Only users with the approver access rights  are allowed to approve please set up in configuration user group')		
        request._add_followers()
        partner_ids = (request.assigned_to.partner_id + request.requested_by.partner_id).ids
        post_vars = {'subject':"notification about purchase request",'body': request.name, 'partner_ids':partner_ids,}	
        request.message_post(type="notification", subtype="mt_comment", **post_vars)
        return request
    ...
    def make_purchase_quotation(self):
        ...
        return dict(name=_('New Quotation'), type='ir.actions.act_window', res_model='purchase.order', view_type='form',
                    view_mode='form', target='new', view_id=view_id.id, views=[(view_id.id, 'form')], context={
